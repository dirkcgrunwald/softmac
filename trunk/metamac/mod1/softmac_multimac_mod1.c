...
static int nullmac_mod1_mytxfunc(void* mydata, struct sk_buff* packet, int intop)
{
	printk("tx mod1\n");
}

static int nullmac_mod1_myrxfunc (void* mydata, struct sk_buff* packet, int intop)
{
	printk("rx mod1\n");
}
...
nullmac_new_instance (void *layer_priv)
{
	...
	inst->macinfo->cu_softmac_mac_packet_rx = nullmac_mod1_myrxfunc;
	inst->macinfo->cu_softmac_mac_packet_tx = nullmac_mod1_mytxfunc;
